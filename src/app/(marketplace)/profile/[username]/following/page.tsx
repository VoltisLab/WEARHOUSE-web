...
import { USER_FOLLOWING } from "@/graphql/queries/social";
...
export default function ProfileFollowingPage() {
  ...
  const { data, loading } = useQuery(USER_FOLLOWING, {
  ...
  useEffect(() => {
    if (ready && !userToken) {
      router.replace(`/login?next=/profile/${encodeURIComponent(rawUsername)}/following`);
    }
  ...
  const users = (data?.following ?? []) as URow[];
  ...
  return (
    <div className="mx-auto max-w-lg space-y-4 pb-28 md:max-w-xl md:pb-12">
      ...
      <h1 className="text-[22px] font-bold text-prel-label">Following</h1>
      {loading ? (
      ...
      {!loading && users.length === 0 ? (
        <p className="text-[14px] text-prel-secondary-label">Not following anyone yet.</p>
      ) : null}
    </div>
  );
}
