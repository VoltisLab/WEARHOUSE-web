...
import { ShopAllBagFloatingBar } from "@/components/marketplace/ShopAllBagFloatingBar";
import { TryCartOnboardingDialog } from "@/components/marketplace/TryCartOnboardingDialog";
...
export function buildBrowseHref(
  ...
  if (opts.tryCart === "1") p.set("tryCart", "1");
...
export function MarketplaceShopBrowse() {
  ...
  const basePath: "/search" | "/shop" = pathname === "/shop" ? "/shop" : "/search";
  const browseFromQuery = searchParams.get("browse");
  const browseParam =
    basePath === "/shop" ? "1" : browseFromQuery;
  ...
  const tryCartParam = searchParams.get("tryCart");

  const isRecentMode = recentParam === "1";
  const isTryCartMode = browseParam === "1" && tryCartParam === "1";

  const [bagToolbarActive, setBagToolbarActive] = useState(false);
  const { add, remove, has } = useShopAllBag();

  useEffect(() => {
    setBagToolbarActive(isTryCartMode);
  }, [isTryCartMode]);
  ...
  const browseTitle = useMemo(() => {
    if (isTryCartMode) return "Try Cart · Shop all";
  ...
  if (isRecentMode && !userToken) {
    return (
      <div className="space-y-4 pb-10">
        <Link
          href="/search"
          className="text-[14px] font-medium text-[var(--prel-primary)]"
        >
          ← Discover
        </Link>
        ...
        <Link
          href="/login"
          className="inline-flex min-h-[48px] items-center justify-center rounded-full bg-[var(--prel-primary)] px-6 text-[15px] font-medium text-white shadow-ios"
        >
          Sign in
        </Link>
      </div>
  ...
  return (
    <>
      {isTryCartMode ? <TryCartOnboardingDialog /> : null}
      {isTryCartMode ? <ShopAllBagFloatingBar /> : null}
      <div
        className={`space-y-6 ${
          isTryCartMode
            ? "pb-[calc(7rem+var(--prel-vv-bottom-inset,0px))]"
            : "pb-8"
        }`}
      >
        <Link
          href="/search"
          className="inline-block text-[14px] font-medium text-[var(--prel-primary)]"
        >
          ← Discover
        </Link>
        ...
        <div>
          ...
          <p className="mt-1 text-[14px] text-prel-secondary-label">
            {isTryCartMode
              ? "Add listings to your bag from different sellers, then open your shopping bag to check out."
          ...
          {isTryCartMode ? (
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <button
                type="button"
                onClick={() => setBagToolbarActive((v) => !v)}
                aria-pressed={bagToolbarActive}
                className={`inline-flex min-h-[44px] items-center gap-2 rounded-full px-4 text-[13px] font-medium shadow-ios ring-1 transition [-webkit-tap-highlight-color:transparent] ${
                  bagToolbarActive
              ...
              >
                ...
                {bagToolbarActive ? "Bag picks on" : "Bag picks off"}
              </button>
              <Link
                href="/saved"
                className="inline-flex min-h-[44px] items-center gap-2 rounded-full bg-white px-4 text-[13px] font-medium text-prel-label shadow-ios ring-1 ring-prel-glass-border transition [-webkit-tap-highlight-color:transparent] hover:bg-prel-bg-grouped"
              >
        ...
        {popularBrands.length > 0 ? (
          <section className="space-y-2">
            <p className="text-[12px] font-medium uppercase tracking-[0.14em] text-prel-secondary-label">
              Popular brands
            </p>
              ...
              {popularBrands.map((b) => {
                ...
                return (
                  <Link
                    key={b.id}
                    href={href}
                    className={`shrink-0 rounded-full px-4 py-2 text-[13px] font-medium shadow-ios ring-1 transition [-webkit-tap-highlight-color:transparent] ${
                      on
                        ? "bg-[var(--prel-primary)] text-white ring-[var(--prel-primary)]"
                        : "bg-white text-prel-label ring-prel-glass-border hover:bg-prel-bg-grouped"
                    }`}
        ...
        <form className="space-y-3" action={basePath} method="get">
          {browseParam === "1" ? (
            <input type="hidden" name="browse" value="1" />
          ) : null}
          {browseParam === "1" && tryCartParam === "1" ? (
            <input type="hidden" name="tryCart" value="1" />
          ) : null}
          ...
          <div className="flex gap-2">
            ...
            <button
              type="submit"
              className="min-h-[48px] shrink-0 rounded-xl bg-[var(--prel-primary)] px-4 text-[15px] font-medium text-white shadow-ios"
            >
              Go
            </button>
          </div>
            ...
            {SORT_OPTIONS.map(({ label, value }) => {
              ...
              return (
                <Link
                  key={value}
                  href={href}
                  className={`inline-flex min-h-[40px] items-center rounded-full px-3.5 py-2 text-[13px] font-medium shadow-ios ring-1 transition [-webkit-tap-highlight-color:transparent] ${
                    on
                      ? "bg-[var(--prel-primary)] text-white ring-[var(--prel-primary)]"
                      : "bg-white text-prel-label ring-prel-glass-border hover:bg-prel-bg-grouped"
                  }`}
        ...
        {brandParam ? (
          <div className="flex items-center justify-between rounded-xl bg-white px-4 py-3 shadow-ios ring-1 ring-prel-glass-border">
            ...
            <Link
              href={buildBrowseHref(basePath, { ...ctxBase, brand: null })}
              className="text-[14px] font-medium text-[var(--prel-primary)]"
            >
              Clear brand
            </Link>
          ...
          {DEPT_CHIPS.map(({ label, value }) => {
            ...
            return (
              <Link
                key={label}
                href={href}
                className={`inline-flex min-h-[44px] items-center rounded-full px-3.5 py-2 text-[13px] font-medium shadow-ios ring-1 transition [-webkit-tap-highlight-color:transparent] active:opacity-90 sm:px-4 ${
                  on
                    ? "bg-[var(--prel-primary)] text-white ring-[var(--prel-primary)]"
                    : "bg-white text-prel-label ring-prel-glass-border hover:bg-prel-bg-grouped"
                }`}
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 md:grid-cols-3 md:gap-5 lg:grid-cols-4 xl:grid-cols-5">
          {rows.map((p) => (
            <MarketplaceProductCard
              ...
              p={p}
              tryCart={
                isTryCartMode && bagToolbarActive
                  ? {
                      enabled: true,
                      inBag: has(p.id),
                      onToggle: () => {
        ...
        <div className="flex justify-center gap-4 pt-2">
          <button
            ...
            className="rounded-full bg-white px-4 py-2 text-[14px] font-medium text-prel-label shadow-ios ring-1 ring-prel-glass-border disabled:opacity-40"
          >
            Previous
          </button>
          <button
