...
import { REPORT_ACCOUNT } from "@/graphql/mutations/account";
...
export function ReportProfileDialog({ username, displayLabel }: Props) {
  const { userToken } = useAuth();
  const [open, setOpen] = useState(false);
  const [reason, setReason] = useState(REASONS[0]);
  const [notes, setNotes] = useState("");
  const [msg, setMsg] = useState<string | null>(null);
  const [reportAccount, { loading }] = useMutation(REPORT_ACCOUNT);
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!userToken) return;
    try {
      const { data, errors } = await reportAccount({
        variables: {
          username,
          reason,
      ...
      setMsg(data?.reportAccount?.message ?? "Report submitted. Thank you.");
      setTimeout(() => {
        setOpen(false);
        setNotes("");
      }, 1200);
      ...
      {open ? (
        ...
        >
          <div className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-2xl bg-white p-6 shadow-xl ring-1 ring-prel-glass-border">
            <h2 id="report-profile-title" className="text-[18px] font-bold text-prel-label">
              Report member
            </h2>
            <p className="mt-1 text-[13px] text-prel-secondary-label">{displayLabel}</p>
